...
- ✅ **Verified vs AI Generated badges** - Each sign card is labeled: "Verified" (green) when the description comes from the curated knowledge base, or "AI generated" (muted outline) when generated by the LLM. Fingerspelled signs show neither — their letter sequence is deterministic regardless. An "AI generated · verify with a native signer" disclaimer appears in every result header
- 🔄 **Grammar Transformation Display** - The gloss bar shows a two-row English → ASL transformation ("English: What is your name?" / "ASL order: YOUR NAME WHAT"), making ASL grammar reordering immediately visible
