...
class TestAnalyticsModel:
    ...
    async def test_ip_hashing(self, test_db: AsyncSession):
        ...
        event1 = await create_analytics_event(
            session=test_db, event_type="translation", ip_address="192.168.1.100"
        )
...
class TestUniqueUsersCount:
    ...
    async def test_get_unique_users_count(self, test_db: AsyncSession):
        ...
        await create_analytics_event(
            test_db, "translation", "192.168.1.1", query="hello"
        )
        await create_analytics_event(
            test_db, "translation", "192.168.1.2", query="world"
        )
        await create_analytics_event(
            test_db, "translation", "192.168.1.1", query="test"
        )  # Duplicate IP
    ...
    async def test_get_unique_users_count_with_date_filter(self, test_db: AsyncSession):
        ...
        event1 = await create_analytics_event(
            test_db, "translation", "192.168.1.1", query="old"
        )
...
class TestTranslationsCount:
    ...
    async def test_get_translations_count(self, test_db: AsyncSession):
        ...
        await create_analytics_event(
            test_db, "translation", "127.0.0.1", query="hello", cache_hit=False
        )
        await create_analytics_event(
            test_db, "translation", "127.0.0.1", query="hello", cache_hit=True
        )
        await create_analytics_event(
            test_db, "translation", "127.0.0.1", query="world", cache_hit=False
        )
        await create_analytics_event(
            test_db, "translation", "127.0.0.1", query="test", cache_hit=True
        )
...
class TestPopularSearches:
    ...
    async def test_get_popular_searches_with_limit(self, test_db: AsyncSession):
        ...
        for i in range(5):
            await create_analytics_event(
                test_db, "translation", "127.0.0.1", query=f"query{i}"
            )
...
class TestHourlyUsagePattern:
    ...
    async def test_get_hourly_usage_pattern(self, test_db: AsyncSession):
        ...
        for i in range(5):
            await create_analytics_event(
                test_db, "translation", f"192.168.1.{i}", query=f"test{i}"
            )
